  ...
  <v-container class="pa-2" fluid grid-list-md>
    <v-layout justify-end>
        <v-btn-toggle v-model="text">
              <v-btn text value="searchbyviewcount" >
                조회수
              </v-btn>
              <v-btn text value="searchbyrating" >
                평점
              </v-btn>
              <v-btn text value="searchbygender" >
                성별
              </v-btn>
               <v-btn text value="searchbyoccupation" >
                직업별
              </v-btn>
              <v-btn text value="searchbyage" >
                연령별
              </v-btn>
          </v-btn-toggle>
    ...
    <v-layout row wrap>
      ...
      <v-pagination v-if="maxPages > 1" v-model="page" :length="maxPages"  :total-visible="11"  dark color="#ff2f6e" background-color="#ff2f6e" circle/>
    </v-layout>
  </v-container>
</template>
...
export default {
  ...
  data: () => ({
    ...
    lists:{
      ...
      searchbyoccupation:["필터 없음","other", "academic/educator", "artist", "clerical/admin", "college/grad student",
       "customer service", "doctor/health care", "executive/managerial",  "farmer",  "homemaker",
         ...
         "unemployed",  "writer"],
      searchbyage:[ "필터 없음","18세 이하","18~24세","25~34세","35~44세","45~49세","50~55세","56세 이상"]
    }
  ...
  computed: {
    ...
    maxPages: function() {
      return Math.floor((this.searchedList.length + this.cardsPerPage - 1) / this.cardsPerPage)
    },
